...
.scroll { background: var(--stage); min-height: 100%; }
...
/* ── the signal chain ──────────────────────────────────────────────────
   The order the engine runs in, as a strip. The matrix below is right for
   editing — it shows every connection you could make — and poor for reading,
...
.chain-node {
  ...
  background: var(--card-alpha); color: var(--foreground);
  border: 1px solid var(--card-border); border-radius: var(--radius-pill);
}
.chain-node:hover { border-color: var(--card-border-hover); }
.chain-node.on { border-color: var(--accent-blue); }
/* Makes audio and sends it nowhere. Nothing said so before, and a block you
   cannot hear looks exactly like one you can. */
.chain-node.unheard { opacity: 0.42; text-decoration: line-through; }
.chain-node.silent { color: var(--muted-foreground); }
.chain-arrow { color: var(--muted-foreground); opacity: 0.5; }
.chain-warn {
  color: var(--accent-amber); font-size: 0.72rem; margin: 0.5rem 0 0;
}
.matrix-row.unheard { opacity: 0.45; }
